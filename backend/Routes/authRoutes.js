...
router.get('/working-hours/:lawyerId', requireAuth, async (req, res) => {
    try {
        const Avocat = require('../Model/Avocat');
        ...
        const avocat = await Avocat.findById(lawyerId).select('workingHours');
        ...
        const convertArrayToObject = (workingHoursArray) => {
            ...
            workingHoursArray.forEach(item => {
                const dayKey = daysMapping[item.day] || item.day.toLowerCase();
                obj[dayKey] = {
                    start: item.start,
                    end: item.end,
        ...
        if (avocat.workingHours && avocat.workingHours.length > 0) {
            workingHours = convertArrayToObject(avocat.workingHours);
        }
...
// Public route to get all verified avocats
router.get('/avocats', async (req, res) => {
    try {
        const Avocat = require('../Model/Avocat');
        
        // Fetch all verified avocats with new profile fields
        const avocats = await Avocat.find({ 
            verified: true 
        }).select('-password -documentsVerif'); // Exclude sensitive fields but include avatarUrl, anneExperience, langues
...
// Public route to get a single avocat by ID
router.get('/avocats/:id', async (req, res) => {
    try {
        const Avocat = require('../Model/Avocat');
        const { id } = req.params;
        
        // Fetch single verified avocat by ID
        const avocat = await Avocat.findOne({ 
            _id: id,
            verified: true 
        }).select('-password -documentsVerif'); // Exclude sensitive fields
